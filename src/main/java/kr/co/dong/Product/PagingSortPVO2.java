...
public class PagingSortPVO2 {
	...
	// 현재페이지, 시작페이지, 끝페이지, 게시글 총 갯수, 페이지당 글 갯수, 마지막페이지, SQL쿼리에 쓸 start, end
		...
		public PagingSortPVO2() {
		}
		public PagingSortPVO2(int total, int nowPage, int cntPerPage, int ptcodemain, int ptcodesub) {
			setNowPage(nowPage);
			setCntPerPage(cntPerPage);
			setTotal(total);
			setPtcodemain(ptcodemain);
			setPtcodesub(ptcodesub);
