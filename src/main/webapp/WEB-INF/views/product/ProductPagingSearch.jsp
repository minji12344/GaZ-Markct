...
<c:forEach var="Product" items="${viewAll}">
	...
	<c:set var="i" value="${i+1 }" />
		...
		<input class="btn btn-primary" type="button" value="상품 등록" id="pinsert" />
		<input class="btn btn-danger" type="button" value="삭제 상품" id="pdellist" />
		<h6>상품 정렬</h6>
  		...
  		<td>
  			<ul class="pagination">
			  	<c:if test="${paging.startPage != 1 }">
			  		<li class="page-item">
						<a href="${contextPath }/Product/ProductPagingSearch?nowPage=${paging.startPage - 1}&cntPerPage=${paging.cntPerPage}" class="page-link">&lt;</a>
					</li>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<li class="page-item">
								<a href="${contextPath }/Product/ProductPagingSearch?nowPage=${p}&cntPerPage=${paging.cntPerPage}" class="page-link">${p }</a>
							</li>
						</c:when>
					</c:choose>
				</c:forEach>
			    <c:if test="${paging.endPage != paging.lastPage}">
			    	<li class="page-item">
						<a href="${contextPath }/Product/ProductPagingSearch?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}" class="page-link">&gt;</a>
					</li>
				</c:if>
			</ul>
  		</td>
